 ...
 * `content/<collection>/<slug>/<lang>.mdx`.
 *
...
const langs: readonly string[] = config.langs
...
function entryFiles(collection: string, slug: string, lang?: string): string[] {
  const base = resolve(contentRoot, collection, slug)
  // A section root is written as `<slug>/index/<lang>.mdx`, a leaf as `<slug>/<lang>.mdx`.
  const names = lang ? [lang, `index/${lang}`, 'index'] : ['index']
  return [
    ...names.flatMap((name) =>
      EXTENSIONS.map((extension) => resolve(base, `${name}.${extension}`))
    ),
...
function sourceFile(pathname: string): string | undefined {
  ...
  const segments = decodeURIComponent(pathname)
    ...
    .filter(Boolean)
  const lang =
    segments[0] && langs.includes(segments[0]) ? segments.shift() : undefined
  const [first, ...rest] = segments

  return [
    // A collection with its own URL prefix: blog, docs, guides…
    ...(first ? entryFiles(first, rest.join('/') || 'index', lang) : []),
    // Anything else is a standalone page.
    ...entryFiles('pages', segments.join('/') || 'index', lang),
