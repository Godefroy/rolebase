---
title: 'role'
description: "Rôle, définit la raison d'être, le domaine, les redevabilités et d'autres aspects d'une position ou fonction au sein d'une organisation. Chaque cercle est associé à un rôle."
...
| `archivedAt` | `Timestamp` | Date d'archivage du rôle ; null si actif |
| `createdAt` | `Timestamp` | Date de création du rôle |
...
- **Modification** — Les propriétaires peuvent modifier tous les rôles. Pour les rôles non-base, l'accès dépend du [`governanceMode`](/fr/docs/governance-modes) de l'organisation : en mode `Free`, les administrateurs et membres peuvent les modifier ; en mode `Agile`, les leaders du cercle le peuvent ; en mode `Strict`, les changements de structure passent par des propositions
