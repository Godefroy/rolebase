---
title: 'thread_activity'
description: "Activité de discussion, représente une activité ou une interaction au sein d'une discussion. Les activités comprennent des messages, des sondages et des références à d'autres entités, constituant le contenu conversationnel d'une discussion."
category: 'threads'
---
...
| `memberId` | `uuid` | Référence au membre qui a créé l'activité |
| `type` | `Thread_Activity_Type_Enum` | Type d'activité : Message, Poll, Proposal, ProposalEvent, Thread, Meeting, Task, Decision ou MeetingNote |
...
| `archivedAt` | `timestamptz` | Date d'archivage de l'activité ; null si actif |
...
- **Proposal** — Proposition de gouvernance avec modifications préparées de l'organigramme
- **ProposalEvent** — Événement de proposition (résolution, annulation, rappel de vote)
...
- `reactions` — Réactions à cette activité (voir [thread_activity_reaction](/fr/developers/graphql-api/thread_activity_reaction))
