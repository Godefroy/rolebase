---
title: 'meeting'
description: "Réunion, représente une réunion planifiée au sein d'un cercle. Les réunions prennent en charge des étapes structurées, le suivi des participants et l'intégration de visioconférence, et peuvent être ponctuelles ou faire partie d'une série récurrente."
category: 'meetings'
---
...
| `archivedAt` | `Timestamp` | Date d'archivage de la réunion ; null si actif |
...
query GetMeetings($orgId: uuid!) {
  meeting(where: { archivedAt: { _is_null: true }, orgId: { _eq: $orgId } }) {
    id
    title
    startDate
    endDate
