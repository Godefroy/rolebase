---
title: 'org'
description: "Organisation, l'entité de niveau supérieur qui contient toutes les autres entités telles que les cercles, rôles, membres, réunions et tâches."
...
| `defaultGraphView` | `String` | Vue par défaut de l'organigramme : `Circles`, `Tree` ou `Members` (optionnel) |
| `defaultGraphFolded` | `Boolean` | Replier la vue par défaut autour du rôle sélectionné (par défaut : false) |
| `defaultGraphMembers` | `Boolean` | Afficher les membres dans les rôles (par défaut : true) |
...
| `onboardingTodo` | `String` | Issue de l'encadré de prise en main : `completed` ou `dismissed`, null tant qu'il est affiché. Modifiable par un Admin ou un Owner |
