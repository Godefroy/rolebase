...
| `view` | `String` | View to render: `Circles`, `Tree` or `Members`. Required. |
| `folded` | `Boolean` | Fold the view around the selected role. Optional, off by default, ignored by `Members`. |
| `width` | `Int` | Image width in pixels, between 100 and 3000. Required. |
| `height` | `Int` | Image height in pixels, between 100 and 3000. Optional, square by default. |
| `colorMode` | `String` | `light` or `dark`. Optional, defaults to `light`. |
| `showMembers` | `Boolean` | List the members inside the roles. Optional, on by default. |
