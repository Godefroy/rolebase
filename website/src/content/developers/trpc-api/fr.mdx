...
Les procédures tRPC s’authentifient avec une **clé API**, la même que celle de l’[API GraphQL](/fr/api). Créez-la depuis [Paramètres > Clés API](/settings/api-keys) dans l’application et envoyez-la dans l’en-tête `x-api-key` :
...
- `superAdmin.impersonate` : délivre un jeton éphémère pour ouvrir une session en tant qu’un autre utilisateur (admin).
