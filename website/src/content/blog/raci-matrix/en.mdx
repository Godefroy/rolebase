...
A RACI covers one project and takes its knowledge with it when the project closes. A role outlives both the project and the person holding it. Teams also reach for a RACI to patch a [matrix organizational structure](/en/blog/matrix-organizational-structure), where the standing question of who decides between the discipline line and the product line has no written answer. That patch expires with the project too. The article on [how to write a role](/en/blog/define-roles) covers the format, and [roles versus job descriptions](/en/blog/roles-vs-job-descriptions) explains why a chart of positions never answers the same question.
