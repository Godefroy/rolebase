...
takeaways:
  ...
  - 'Letting each department differ strongly from the next only works when liaison roles and arbitration bodies hold them back together. The companies that do one without the other perform worse than the ones that do both.'
...
It works when the two lines have an explicit rule for the case where they disagree. Without that rule the conflict lands on whoever works under both. A call that needs two signatures can then be stalled by either side, which Stanley Davis and Paul Lawrence, who wrote the 1977 reference book on the form, called decision strangulation in the *Harvard Business Review* of May 1978. The weak, balanced and strong variants, the nine ways the form fails and what a company can write down instead of hiring a second manager are covered in [the article on the matrix organizational structure](/en/blog/matrix-organizational-structure).

## The hierarchical structure

A hierarchical structure stacks levels of authority, each level supervising a narrow group, with a single line of command from top to bottom. It is Fayol's answer, and it has run companies for a hundred and seventy years. You draw it as a [hierarchy chart](/en/blog/hierarchy-chart), with each position under the one it reports to.
