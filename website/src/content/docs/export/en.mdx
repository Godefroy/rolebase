...
The export page offers the role to capture, the view to use and whether to fold it, the image width and the **Show members** option. With that option off, members are absent from the image and reserve no room for themselves: the roles tighten up accordingly. The preview shows exactly what the file will contain, and the page address keeps these settings, to share or come back to.
