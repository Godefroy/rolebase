...
- Never run codegen yourself (`graphql-codegen` / `npm run codegen`). It runs continuously in the dev server the user keeps running alongside; editing `.gql` files regenerates the GraphQL types automatically (`packages/webapp/src/graphql.generated.ts`, `packages/shared/graphql/`, `packages/backend/src/graphql/`). Just wait for it.
...
- One React component per file, always. Even a small helper/presentational sub-component goes in its own file in the appropriate `components/` (or `modals/`) folder.
- Keep props minimal. Many props means the decomposition is wrong: lift shared state/actions into a feature React context (`*Provider` + `use*` hook) that sub-components consume with few/zero props. Co-locate each modal with the component that triggers it; never build one component that renders all modals.
...
- Always check whether the doc needs updating before committing. Only pause to ask the user to review when **you edited the doc in response to the current commit instruction** (i.e. after they said "commit"). If you make such edits, do not create the commit straight away: first ask the user to review the doc changes, and only create the commit once they confirm. If the doc was already edited and presented in an earlier turn, or no doc edit is needed, commit directly without asking.
